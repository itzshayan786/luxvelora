...
const CheckoutPage = () => {
  ...
  const placeOrder = async () => {
    ...
    setPlacing(false)
    if (r.error) return toast.error(r.error)
    clearCart(); setRoute({ view: 'order-success', order: r.order })
  }

  useEffect(() => { if (cart.length === 0) setRoute({ view: 'cart' }) }, [cart])
              ...
              {payment === 'upi' && (() => {
                ...
                return (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-strong rounded-2xl p-6 border border-black/10">
                    ...
                    <p className="text-sm text-neutral-500 mb-5">Scan the QR with any UPI app, or copy the UPI ID.</p>
                    ...
                    <div className="mt-6 pt-5 border-t border-black/5">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[
                          ...
                          <span key={a.n} className={`text-[11px] font-semibold px-3 py-1.5 rounded-full text-white bg-gradient-to-br ${a.c}`}>{a.n}</span>
                        ))}
                      </div>
                      <div className="flex items-start gap-2 mt-5 p-3 rounded-xl bg-amber-50 border border-amber-200">
                        <Sparkles className="w-4 h-4 text-amber-600 flex-shrink-0 mt-0.5" />
                        <p className="text-xs text-amber-900">After paying via UPI, click <b>Continue</b> to review and place your order. We'll auto-verify the payment.</p>
                      </div>
                    </div>
                  </motion.div>
                )
