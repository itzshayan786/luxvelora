...
const openRazorpayCheckout = async ({ order, description, image, prefill, notes, onVerified, onDismiss }) => {
  ...
  const placeRazorpayOrder = async () => {
    ...
    try {
      const orderRes = await fetch('/api/create-order', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ items: cart.map(i => ({ id: i.id, quantity: i.qty, size: i.size, color: i.color })) }),
      })
      const order = await orderRes.json()
      ...
      await openRazorpayCheckout({
        ...
        onVerified: async (verified) => {
          const orderBody = { items: cart, address: form, email: form.email, name: form.name, phone: form.phone, payment: 'razorpay', coupon: applied?.code, subtotal, shipping, discount, total, razorpayOrderId: verified.order_id, razorpayPaymentId: verified.payment_id }
          await fetch('/api/checkout', { method: 'POST', body: JSON.stringify(orderBody) })
          clearCart()
          redirectToPaymentResult('/payment/success', {
            order_id: verified.order_id,
            payment_id: verified.payment_id,
