...
const VELORA_THEME_COLOR = '#2563eb'

const loadRazorpayScript = () => new Promise((resolve, reject) => {
  if (typeof window === 'undefined') return reject(new Error('Razorpay checkout is unavailable'))
...
const redirectToPaymentResult = (path, params) => {
  const query = new URLSearchParams(params)
  window.location.href = `${path}?${query.toString()}`
}

const openRazorpayCheckout = async ({ order, description, image, prefill, notes, orderPayload, onVerified, onDismiss }) => {
  ...
  return new Promise((resolve) => {
    const checkout = new window.Razorpay({
      ...
      prefill,
      notes,
      theme: { color: VELORA_THEME_COLOR },
      handler: async (response) => {
        try {
          ...
          if (verifyRes.ok && verified.success) {
            await onVerified(verified, response)
            resolve(verified)
            return
          }

          redirectToPaymentResult('/payment/failed', {
            order_id: response.razorpay_order_id || order.order_id,
            reason: verified.error || 'verification_failed',
          })
          resolve(null)
        } catch (error) {
          redirectToPaymentResult('/payment/failed', {
    ...
    checkout.on('payment.failed', (response) => {
      redirectToPaymentResult('/payment/failed', {
        order_id: response.error?.metadata?.order_id || order.order_id,
...
const ProductPage = () => {
  const { route, setRoute, addToCart, toggleWishlist, wishlist, openBuyNow } = useShop()
    ...
    <div className="pt-8 pb-20 px-6 md:px-12 max-w-[1600px] mx-auto">
      ...
      <div className="grid md:grid-cols-2 gap-8 lg:gap-16">
        ...
        <div>
          ...
          <Button onClick={() => openBuyNow(product, size, color, qty)} className="w-full h-14 rounded-full bg-blue-600 text-white hover:bg-neutral-900 hover:text-white font-semibold shadow-lg shadow-blue-600/20 mb-8">
            Buy Now <CreditCard className="w-4 h-4 ml-2" />
          </Button>
...
const OrderSummaryModal = ({ item, onClose }) => {
  const { user } = useShop()
  ...
  const mrp = Number(product.mrp || product.price) * qty
  ...
  const proceedToPayment = async () => {
    ...
    try {
      ...
      await openRazorpayCheckout({
        ...
        prefill: {
          name: user?.name || 'Velora Customer',
          email: user?.email || '',
          contact: user?.phone || '',
        },
        notes: {
        ...
        orderPayload: {
          items: [{ id: product.id, name: product.name, price: Number(product.price), mrp: Number(product.mrp || product.price), image: product.images?.[0] || '', size, color, qty, quantity: qty }],
          address: null,
          email: user?.email || '',
          name: user?.name || '',
          phone: user?.phone || '',
        ...
        onVerified: async (verified) => {
          redirectToPaymentResult('/payment/success', {
            order_id: verified.order_id,
            payment_id: verified.payment_id,
            order_ref: verified.order?.id || '',
          })
        },
      })
    } catch (error) {
  ...
  return (
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[120] bg-neutral-950/70 backdrop-blur-sm px-4 py-6 flex items-center justify-center">
        <motion.div initial={{ opacity: 0, y: 24, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 16, scale: 0.98 }} transition={{ type: 'spring', damping: 24 }} className="glass-strong w-full max-w-2xl rounded-3xl overflow-hidden border border-white/30 shadow-2xl">
          <div className="relative bg-neutral-950 text-white p-6">
            <div className="aurora opacity-40" />
            <button onClick={onClose} className="absolute right-5 top-5 w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center"><X className="w-4 h-4" /></button>
            <p className="relative text-[11px] tracking-[0.35em] text-blue-300 mb-2">SECURE CHECKOUT</p>
            <h2 className="relative text-3xl font-display font-bold silver-white">Order Summary</h2>
            <p className="relative text-sm text-white/60 mt-2">Review your piece before entering Razorpay's encrypted checkout.</p>
          </div>
          <div className="p-6 md:p-8">
            <div className="flex gap-5">
              <img src={product.images?.[0]} alt={product.name} className="w-28 h-36 md:w-36 md:h-44 rounded-2xl object-cover bg-black/[0.03]" />
              <div className="flex-1 min-w-0">
                <h3 className="text-xl md:text-2xl font-display font-bold text-neutral-950">{product.name}</h3>
                <div className="grid grid-cols-2 gap-3 mt-5 text-sm">
                  <div><p className="text-neutral-500 text-xs tracking-[0.2em]">COLOR</p><p className="font-medium">{color}</p></div>
                  <div><p className="text-neutral-500 text-xs tracking-[0.2em]">SIZE</p><p className="font-medium">{size}</p></div>
                  <div><p className="text-neutral-500 text-xs tracking-[0.2em]">QUANTITY</p><p className="font-medium">{qty}</p></div>
                  <div><p className="text-neutral-500 text-xs tracking-[0.2em]">MRP</p><p className="font-medium">{fmt(mrp)}</p></div>
                </div>
              </div>
            </div>
            <div className="hair-line my-6" />
            <div className="space-y-3 text-sm">
              <div className="flex justify-between text-neutral-700"><span>MRP</span><span>{fmt(mrp)}</span></div>
              <div className="flex justify-between text-blue-500"><span>Discount</span><span>-{fmt(discount)}</span></div>
              <div className="flex justify-between text-neutral-700"><span>Shipping</span><span>{shipping === 0 ? 'FREE' : fmt(shipping)}</span></div>
              <div className="flex justify-between text-xl font-bold pt-3 border-t border-black/10"><span>Total</span><span className="silver-text">{fmt(total)}</span></div>
            </div>
            <Button disabled={paying} onClick={proceedToPayment} className="mt-7 w-full h-14 rounded-full bg-neutral-900 text-white hover:bg-blue-600 hover:text-white font-semibold text-base">
              {paying ? <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Opening Secure Payment...</> : <><Shield className="w-5 h-5 mr-2" /> Proceed to Secure Payment</>}
            </Button>
            <p className="mt-4 text-center text-xs text-neutral-500">UPI, cards, net banking and wallets are enabled through Razorpay Standard Checkout.</p>
          </div>
        </motion.div>
      </motion.div>
    </AnimatePresence>
  ...
  const placeRazorpayOrder = async () => {
    ...
    try {
      const orderRes = await fetch('/api/create-order', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ items: cart.map(i => ({ id: i.id, quantity: i.qty, size: i.size, color: i.color })), coupon: applied?.code }),
      })
      const order = await orderRes.json()
      if (!orderRes.ok || order.error) throw new Error(order.error || 'Unable to create Razorpay order')

      await openRazorpayCheckout({
        order,
        description: `${cart.length} item${cart.length > 1 ? 's' : ''} from Velora`,
        prefill: { name: form.name, email: form.email, contact: form.phone },
        notes: { checkout: 'cart' },
        orderPayload: { items: cart, address: form, email: form.email, name: form.name, phone: form.phone, payment: 'razorpay', coupon: applied?.code, subtotal, shipping, discount, total },
        ...
        <div className="lg:col-span-2">
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div className="glass rounded-2xl p-6 border border-black/10 space-y-3">
                ...
                {[
                  { v: 'razorpay', l: 'Razorpay Checkout', s: 'UPI, cards, netbanking and wallets via Razorpay', icon: CreditCard },
          ...
          <div className="flex gap-3 mt-6">
            ...
            {step < 3 ? (
              ...
              <Button disabled={placing} onClick={placeOrder} className="rounded-full bg-blue-500 hover:bg-blue-400 text-white ml-auto h-12 px-8">{placing ? 'Processing...' : payment === 'razorpay' ? `Proceed to Secure Payment · ${fmt(total)}` : `Place Order · ${fmt(total)}`}</Button>
            )}
          </div>
        </div>
...
const App = () => {
  ...
  const openBuyNow = (product, size, color, qty = 1) => setBuyNowItem({ product, size, color, qty })
  ...
  const ctx = { route, setRoute, products, cart, addToCart, updateCart, removeFromCart, clearCart, openBuyNow, wishlist, toggleWishlist, user, setUser, searchOpen, setSearchOpen, mobileNavOpen, setMobileNavOpen }
  ...
  return (
    <ShopCtx.Provider value={ctx}>
      <div className="min-h-screen bg-[#fafaf9] noise">
        ...
        <Footer />
        <SearchOverlay />
        <OrderSummaryModal item={buyNowItem} onClose={() => setBuyNowItem(null)} />
        <AIChatWidget />
      </div>
    </ShopCtx.Provider>
  )
